...
read_sheet("1tlqAVKrW1HbeeB7wbUB6jMlN1jQoNYUtDk0WnL7_G_A", sheet = "edu") %>%
  detailed_entries(when = glue("{month(start_date,T)} {year(start_date)}--{month(end_date,T)} {year(end_date)}"),   
                   what = degree,
                   with = loc,
                   where = uni,
                   why = details)
...
pubs = bind_rows(pubmed, biorxiv) %>% 
  ...
  mutate(authors = case_when(doi == "10.7554/eLife.06416" ~ glue("<<authors>> {\\\\\\underline{MS Cuoco} listed at co-investigator}", .open = "<<", .close = ">>"),
